 ...
 */
const Datasets = ({ hostValue, updateTomatoes }) => {
    ...
    useEffect(() => {
        /*-----------------------------------------------------------------------------------------------------------------*\
            # 웹소켓 커넥션하는 동안 데이터를 수신하며, 데이터를 배열에 저장하고 필요한 형식으로 변환하여 차트 데이터를 업데이트 #
        \*-----------------------------------------------------------------------------------------------------------------*/
        const ws = new WebSocket("ws://localhost:8001/healthy");
...
const TomatoClassification = ({ tomatoes }) => {
    return <OpenAI tomatoesStatus={tomatoes} />
}

const StreamChart = ({ hostValue }) => {
    const [tomatoes, setTomatoes] = useState({});
    const handleHostValue = (value) => {
        hostValue(value);
    };

    const updateTomatoes = (data) => {
        setTomatoes(data);
    };

    return (
        <div className="harvest-box">
            <Datasets hostValue={handleHostValue} updateTomatoes={updateTomatoes} />
            <TomatoClassification tomatoes={tomatoes} />
        </div>
    );
};
