...
const Calendar = ({ toChangePost, intoChangePost, intoChangeStatus }) => {
  ...
  const { setVisible, visible, setDate, date, setCurrentDate, currentDate } =
    useCalendarEvents();
  ...
  useEffect(() => {
    setCurrentDate({
      year: date?.year,
      month: date?.month - 1,
      string_month: new Date(date?.year, date?.month - 1).toLocaleString(
        "en-GB",
        { month: "long" }
      ),
    });
  ...
  const [month_change_style, set_month_change_style] = useState({
    visibility: "hidden",
  });
  ...
  const prevMonth = () => {
    ...
    let newStringMonth = new Date(newYear, newMonth).toLocaleString("en-GB", {
      month: "long",
    });
  ...
  useEffect(() => {
    ...
    for (
      let i = 1;
      i <= total_days - (prev_month_days.length + days_array.length);
      i++
    ) {
  ...
  const host_month = (value) => {
    setOnDate({
      year: onDate.year,
      month: value,
      string_month: new Date(onDate.year, value).toLocaleString("en-GB", {
        month: "long",
      }),
    });
  ...
  const isActiveDay = (day) => {
    ...
    return (
      onDate.year === accessDate.getFullYear() &&
  ...
  const getEventClass = (day) => {
    ...
    for (const event of events) {
      ...
      if (
        currentDateString === startDate.toDateString() &&
        currentDateString === endDate.toDateString()
      ) {
                ...
                {prev_last_days.map((o, i) => (
                  <li
                    ...
                    onClick={(e) => {
                      ...
                      setDate({
                        year:
                          onDate.month === 0 ? onDate.year - 1 : onDate.year,
          ...
          <div className="month-list" style={{ transform: `scale(${scale})` }}>
            {month_list.map((month) => (
              <div key={month}>
                <div
                  className={
                    onDate.year === new Date().getFullYear() &&
