 ...
 */
export const NotificationBox = ({ popupStatus, enterEvent }) => {
  const { getNotification, updateNotification, deleteNotification } =
    useNoticeEvents();
  ...
  useEffect(() => {
    if (!enterEvent) {
      ...
      return () => {
        document.removeEventListener("mousedown", popupClose);
      };
    }
  ...
  const icon = (type) => {
    switch (type) {
      case "gpt":
        return <AiOutlineOpenAI className="item" />;
      case "warning":
        return <FontAwesomeIcon icon={faCircleExclamation} className="item" />;
      default:
        return <FontAwesomeIcon icon={faMessage} className="item" />;
    }
  };
  ...
  return (
    <div ref={wrapperRef}>
      <section
        className={`notice-box ${isOpened ? "visible" : ""}`}
        ref={wrapperRef}
      >
        {/**
            ...
            return (
              <div key={notification._id} className="notice-item">
                <div className="notice-icon">
                  <div className="sort">{icon(notification.type)}</div>
                </div>
                <div className="contents">
                  <h4 className="notice-time">
                    {moment(notification.time).calendar(null, {
                      sameDay: "[오늘] HH:mm",
                      lastDay: "[어제] HH:mm",
                      lastWeek: "dddd HH:mm",
